...
[![Zotero 插件版本](https://img.shields.io/badge/Zotero%20插件-v1.6.0-2563eb)](https://github.com/groele/Mindflow-zotero/releases/tag/v1.6.0)
...
> **版本说明：**本仓库同时保留 Chrome 扩展源码（版本 3.2.0）。本 README 和 GitHub `v1.6.0` 标签针对 **Zotero 插件**；下载 Zotero 安装包时请选择 `mindflow-zotero-1.6.0.xpi`，不要选择 Chrome ZIP。

## 下载与安装

1. 从 [v1.6.0 发布页](https://github.com/groele/Mindflow-zotero/releases/tag/v1.6.0)的 Assets 下载 `mindflow-zotero-1.6.0.xpi`。
...
安装文件为 `dist-zip/mindflow-zotero-1.6.0.xpi`；已解压的构建目录在 `dist-zotero/`。这两个目录是生成物，不提交到 Git。Chrome 扩展另用 `npm run build` 构建到 `dist/`，其版本仍为 3.2.0。
