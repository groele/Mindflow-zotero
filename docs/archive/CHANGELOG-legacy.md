...
- **工具栏按钮自由显隐**：支持独立开关 10 项按钮（撤销重做、插入分支、布局切换、主题切换、大纲、收集箱、模板库、专注模式、导出、缩放控件），避免小屏/Side Panel 拥挤。
...
- **任务默认优先级**：可设置新建 Task 时的默认优先级（P1/P2/P3）。
...
- **停靠位置自由对调 (`dockPosition`)**：
  - 支持一键将宏观工作台停靠在屏幕左端或右端（自动保存至 `mindflow_dock_pos`），特别适配 Chrome 侧边栏（Side Panel）无论停靠在浏览器左还是右的场景。
